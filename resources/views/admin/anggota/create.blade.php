...
<body class="bg-slate-50 font-sans antialiased text-slate-800 flex h-screen overflow-hidden selection:bg-blue-200 selection:text-blue-900">
    ...
    <aside class="w-64 bg-[#0f172a] text-slate-300 flex flex-col h-full shadow-2xl shrink-0 transition-all duration-300 z-20">
        <div class="h-20 flex items-center px-6 border-b border-slate-700/50 bg-[#0f172a] shrink-0">
            <img src="{{ asset('images/logo-sd.png') }}" class="h-8 w-8 object-contain mr-3 bg-white rounded-full p-0.5" onerror="this.src='https://placehold.co/100x100/ffffff/1e293b?text=SD'">
    ...
    <div class="flex-1 flex flex-col h-full overflow-hidden w-full relative">
        <header class="h-20 bg-white shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] flex items-center justify-between px-8 z-10 shrink-0">
            <div>
                ...
                <h2 class="text-2xl font-bold text-slate-800 tracking-tight leading-none">Tambah Anggota Baru</h2>
            </div>
            ...
            <form action="{{ route('admin.anggota.store') }}" method="POST" enctype="multipart/form-data" class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
                ...
                <div class="lg:col-span-3 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden sticky top-0">
                    <div class="bg-slate-50 px-5 py-4 border-b border-slate-100 flex items-center gap-2">
                        ...
                        <h3 class="font-bold text-slate-800 text-sm">Foto Profil</h3>
                    </div>
                    <div class="p-5 flex flex-col items-center justify-center">
                        <div class="w-full aspect-[3/4] border-2 border-dashed border-slate-300 rounded-xl flex flex-col items-center justify-center bg-slate-50 hover:bg-blue-50 transition-colors relative overflow-hidden group">
                            <input type="file" id="foto" name="foto" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                            <div class="text-center p-4">
                                <i class="fa-solid fa-cloud-arrow-up text-3xl text-slate-300 group-hover:text-blue-500 mb-3"></i>
                                <p class="text-xs text-slate-500 font-medium leading-relaxed group-hover:text-blue-600">Klik untuk<br>pilih gambar</p>
                            </div>
                        </div>
                        <p class="text-[11px] font-semibold text-rose-500 mt-4 text-center">Format: JPG/PNG.<br>Maks. 2MB</p>
                    </div>
                </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 md:gap-4 items-center">
                                ...
                                <div class="md:col-span-2 flex items-center gap-6">
                                    <label class="flex items-center gap-2 text-sm text-slate-700 font-medium">
                                        <input type="radio" name="jenis_kelamin" value="Laki-laki" {{ old('jenis_kelamin', 'Laki-laki') == 'Laki-laki' ? 'checked' : '' }} class="w-4 h-4 text-blue-600"> Laki-laki
                                    </label>
                                    <label class="flex items-center gap-2 text-sm text-slate-700 font-medium">
                                        <input type="radio" name="jenis_kelamin" value="Perempuan" {{ old('jenis_kelamin') == 'Perempuan' ? 'checked' : '' }} class="w-4 h-4 text-blue-600"> Perempuan
                                    </label>
                                </div>
                            </div>
