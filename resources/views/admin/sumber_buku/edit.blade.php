...
<head>
    ...
    <title>Edit Sumber Buku - SMARTPERPUS</title>
    ...
    <aside class="w-64 bg-[#0f172a] text-slate-300 flex flex-col h-full shadow-2xl shrink-0 transition-all duration-300 z-20">
        ...
        <nav class="flex-1 overflow-y-auto py-6 px-3">
            <ul class="space-y-1.5">
                ...
                <li>
                    <a href="/admin/sumber-buku" class="flex items-center px-4 py-3 bg-amber-600/10 text-amber-500 border border-amber-500/20 rounded-xl font-semibold shadow-inner transition-all">
        ...
        <header class="h-20 bg-white shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] flex items-center justify-between px-8 z-10 shrink-0">
            <div>
                <div class="flex items-center text-sm text-slate-500 mb-1 font-semibold gap-2">
                    <a href="/admin/sumber-buku" class="hover:text-blue-600">Sumber Buku</a>
                    <i class="fa-solid fa-chevron-right text-[10px]"></i>
                    <span class="text-slate-800">Edit Data</span>
                </div>
                <h2 class="text-2xl font-bold text-slate-800 tracking-tight leading-none">Edit Sumber Buku</h2>
            </div>
        </header>
            ...
            <div class="max-w-4xl mx-auto space-y-5">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                    <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-2 bg-white">
                        ...
                        <h3 class="font-bold text-slate-800 text-base">Ubah Data Sumber Buku</h3>
                    </div>

                    <form action="/admin/sumber-buku/{{ $sumber->id }}" method="POST" class="p-6 md:p-8">
                        @csrf @method('PUT')
                        <div class="space-y-6 max-w-3xl">
                            <!-- Kode Sumber -->
                            <div class="grid grid-cols-1 md:grid-cols-4 md:gap-4 items-center">
                                <label class="text-sm font-semibold text-slate-600 mb-1 md:mb-0">Kode Sumber <span class="text-rose-500">*</span></label>
                                <div class="md:col-span-3">
                                    <input type="text" name="kode_sumber" value="{{ old('kode_sumber', $sumber->kode_sumber) }}" class="w-full bg-slate-50 border border-slate-200 rounded py-2.5 px-4 text-sm focus:outline-none focus:border-amber-500 uppercase font-mono" required>
                                </div>
                            </div>
                            <!-- Nama Sumber -->
                            <div class="grid grid-cols-1 md:grid-cols-4 md:gap-4 items-center">
                                <label class="text-sm font-semibold text-slate-600 mb-1 md:mb-0">Nama Sumber <span class="text-rose-500">*</span></label>
                                <div class="md:col-span-3">
                                    <input type="text" name="nama_sumber" value="{{ old('nama_sumber', $sumber->nama_sumber) }}" class="w-full bg-slate-50 border border-slate-200 rounded py-2.5 px-4 text-sm focus:outline-none focus:border-amber-500" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 pt-8 border-t border-slate-100 mt-8 max-w-3xl">
                            ...
                            <div class="w-full flex gap-3">
                                ...
                                <a href="/admin/sumber-buku" class="bg-[#212529] hover:bg-[#1c1f23] text-white px-6 py-2.5 rounded shadow-sm font-medium text-sm flex items-center justify-center gap-2"><i class="fa-solid fa-xmark"></i> Batal</a>
                            </div>
                        </div>
                    </form>
                </div>
