    ...
    <aside class="w-64 bg-[#0f172a] text-slate-300 flex flex-col h-full shadow-2xl shrink-0 transition-all duration-300 z-20">
        ...
        <nav class="flex-1 overflow-y-auto py-6 custom-scrollbar">
            <ul class="space-y-1.5 px-3">
                ...
                <!-- Sumber Buku Active -->
                <li>
                    <a href="{{ route('admin.sumber_buku.index') }}" class="flex items-center px-4 py-3 bg-blue-600/10 text-blue-400 border border-blue-500/20 rounded-xl font-semibold shadow-inner transition-all">
    ...
    <div class="flex-1 flex flex-col h-full overflow-hidden w-full relative">
        ...
        <header class="h-20 bg-white shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] flex items-center justify-between px-8 z-10 shrink-0">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Kelola data asal usul koleksi buku perpustakaan.</p>
            </div>
            ...
            <div class="max-w-6xl mx-auto space-y-6">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
                    ...
                    <div class="flex items-center gap-2 bg-white px-4 py-2.5 rounded-lg border border-slate-200 shadow-sm shrink-0">
                        ...
                        <span class="font-bold text-slate-800 text-sm">Daftar Sumber Buku</span>
                    </div>

                    <!-- Search Bar & Add Button -->
                    <div class="flex items-center gap-3 w-full md:w-auto">
                        <div class="relative w-full md:w-64">
                            <input type="text" id="searchInput" placeholder="Cari kode atau nama sumber..." class="w-full bg-white border border-slate-200 rounded-lg py-2.5 px-4 pl-10 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-slate-700 outline-none shadow-sm">
                            <i class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"></i>
                        </div>

                        <!-- Tombol Tambah yang mengarah ke create.blade.php -->
                        <a href="{{ route('admin.sumber_buku.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg shadow-sm text-sm font-semibold transition-colors flex items-center gap-2 shrink-0">
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-100/80 text-slate-500 text-[11px] uppercase tracking-widest border-b border-slate-200">
                                    ...
                                    <th class="px-6 py-4 font-bold w-48">Kode Sumber</th>
                                    <th class="px-6 py-4 font-bold">Nama Sumber Buku</th>
                                    <th class="px-6 py-4 font-bold w-32 text-center">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="text-slate-700 text-sm divide-y divide-slate-100" id="tableBody">
                                ...
                                <tr class="hover:bg-slate-50 transition-colors table-row-item">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center gap-1.5">
                                            <!-- Tombol Edit -->
                                            <a href="{{ route('admin.sumber_buku.edit', $item->id) }}" class="w-8 h-8 rounded bg-amber-500 hover:bg-amber-600 text-white flex items-center justify-center transition-colors shadow-sm tooltip" title="Edit Data">
                                                <i class="fa-solid fa-pen-to-square text-[13px]"></i>
                                            </a>
                                            <!-- Tombol Hapus Panggil Modal -->
                                            <button type="button" onclick="openGlobalDeleteModal('{{ route('admin.sumber_buku.destroy', $item->id) }}', '{{ $item->nama_sumber }}')" class="w-8 h-8 rounded bg-rose-500 hover:bg-rose-600 text-white flex items-center justify-center transition-colors shadow-sm tooltip" title="Hapus Data">
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center text-slate-400">
                                            ...
                                            <p class="font-medium text-slate-500">Belum ada data sumber buku.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
    ...
    <div id="globalDeleteModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center opacity-0 pointer-events-none transition-opacity duration-300">
        <div id="globalDeleteModalContent" class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 sm:p-8 transform scale-95 opacity-0 transition-all duration-300 m-4">
            <div class="flex flex-col items-center text-center">
                ...
                <h3 class="text-xl font-bold text-slate-800 mb-2">Hapus Data Sumber Buku?</h3>
