        ...
        <header class="h-20 bg-white shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] flex items-center justify-between px-8 z-10 shrink-0">
            <div>
                <div class="flex items-center text-sm text-slate-500 mb-1 font-semibold gap-2">
                    ...
                    <a href="{{ route('admin.kelas.index') }}" class="hover:text-blue-600 cursor-pointer transition-colors">Data Kelas</a>
                    <i class="fa-solid fa-chevron-right text-[10px]"></i>
            ...
            <div class="flex items-center gap-4 md:gap-6">
                <img src="https://ui-avatars.com/api/?name=Admin+Perpus&background=0f172a&color=fff&bold=true" class="w-10 h-10 rounded-full ring-2 ring-slate-100">
            </div>
        </header>
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto p-4 md:p-8 custom-scrollbar">
            <div class="max-w-[1400px] mx-auto space-y-5">
                ...
                <div class="bg-amber-50 text-amber-800 rounded-lg p-5 shadow-sm border border-amber-200 flex flex-col justify-center">
                    <div class="flex items-center gap-2 mb-1.5"><i class="fa-solid fa-pen-to-square text-xl opacity-90"></i><h3 class="font-bold text-xl">Ubah Informasi</h3></div>
                    <p class="text-sm text-amber-700">Anda sedang mengubah data kelas: <strong>{{ $kelas->nama_kelas }}</strong>.</p>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                    <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-2 bg-white">
                        ...
                        <h3 class="font-bold text-slate-800 text-base">Form Edit Data Kelas <span class="text-rose-500">(*)</span></h3>
                    </div>

                    <form action="{{ route('admin.kelas.update', $kelas->id) }}" method="POST" class="p-6 md:p-8">
                        @csrf @method('PUT')
                        <div class="space-y-6 max-w-3xl">
                            <div class="grid grid-cols-1 md:grid-cols-4 md:gap-4 items-center">
                                ...
                                <div class="md:col-span-3">
                                    <input type="text" name="nama_kelas" value="{{ old('nama_kelas', $kelas->nama_kelas) }}" class="w-full bg-slate-50 border border-slate-200 rounded py-2.5 px-4 text-sm focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-amber-500 transition-all text-slate-700 shadow-sm" required autofocus>
                                </div>
                            </div>
                        </div>

                        <div class="flex items-center gap-3 pt-6 border-t border-slate-100 max-w-3xl mt-8">
                            ...
                            <div class="w-full flex gap-3">
                                ...
                                <a href="{{ route('admin.kelas.index') }}" class="bg-[#212529] hover:bg-[#1c1f23] text-white px-6 py-2.5 rounded shadow-sm transition-colors flex items-center justify-center gap-2 font-medium text-sm flex-1 md:flex-none">
