            ...
            <div class="max-w-[1400px] mx-auto space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                    <div class="px-6 py-4 flex items-center justify-between bg-white">
                        ...
                        <a href="{{ route('admin.buku.create') }}" class="bg-[#3b5998] hover:bg-[#2d4373] text-white px-4 py-2 rounded flex items-center justify-center transition-colors shadow-sm gap-2 text-sm font-semibold">
                            <i class="fa-solid fa-plus"></i> Tambah Buku Baru
                        </a>
                    </div>

                    <div class="px-6 py-3 border-t border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                        ...
                        <div class="relative w-full md:w-80">
                            ...
                            <input type="text" id="searchInput" placeholder="Cari Judul / Pengarang / Penerbit..." class="w-full bg-white border border-slate-300 rounded py-2 pl-9 pr-3 text-sm focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-700 outline-none shadow-sm">
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse" id="dataTable">
                            ...
                            <tbody id="tableBody" class="text-slate-700 text-[13px] divide-y divide-slate-100">
                                ...
                                @forelse ($data_buku as $index => $item)
                                <tr class="hover:bg-slate-50 transition-colors {{ $loop->iteration % 2 == 0 ? 'bg-slate-50/50' : 'bg-white' }}">
                                    ...
                                    <!-- QR Code Otomatis Berdasarkan Kode Buku -->
                                    <td class="px-5 py-3 text-center">
                                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=60x60&data={{ $item->kode_buku }}" alt="QR Code" class="w-10 h-10 mx-auto mix-blend-multiply border border-slate-200 p-0.5 rounded bg-white">
                                    </td>

                                    <td class="px-5 py-3 font-semibold text-slate-800">
                                        {{ $item->judul }}
                                        <div class="text-[10px] text-slate-500 font-normal mt-0.5"><span class="font-mono">{{ $item->kode_buku }}</span> | Kategori: {{ $item->kategori->nama_kategori ?? '-' }}</div>
                                    </td>

                                    <td class="px-5 py-3 text-slate-600">{{ $item->pengarang }}</td>
                                    <td class="px-5 py-3 text-slate-600">{{ $item->penerbit }}</td>
                                    <td class="px-5 py-3 text-center text-slate-600 font-medium">{{ $item->tahun_terbit }}</td>

                                    <td class="px-5 py-3 text-center">
                                        <!-- Logika untuk menampilkan gambar dari storage atau default -->
                                        @if($item->cover)
                                            <img src="{{ asset('storage/covers/'.$item->cover) }}" alt="Cover" class="w-9 h-12 object-cover mx-auto border border-slate-200 shadow-sm rounded">
                                        @else
                                            <img src="https://placehold.co/150x200?text=No+Cover" alt="Cover" class="w-9 h-12 object-cover mx-auto border border-slate-200 shadow-sm rounded">
                                        @endif
                                    </td>
                        ...
                        <div id="noMatchMessage" class="hidden text-center py-8 text-slate-500 font-medium">
